...
while read user; do
	...
	if [[ $expired < $today ]]; then
		userdel $user
		cat /phantom/user_database.json | jq 'del(.ssh[] | select(.username == "'$user'"))' > /phantom/user_database.json.tmp
...
while read user; do
	[[ -z $user ]] && break
	expired=$(cat /phantom/user_database.json | jq -r '(.wireguard[] | select(.username == "'$user'")).expired')
	if [[ $expired < $today ]]; then
		...
		rm -f /phantom/wireguard/${user}.conf
		cat /phantom/user_database.json | jq 'del(.wireguard[] | select(.username == "'$user'"))' > /phantom/user_database.json.tmp
		mv /phantom/user_database.json.tmp /phantom/user_database.json
	fi
done <<< "$(cat /phantom/user_database.json | jq -r '.wireguard[].username')"
...
while read user; do
	...
	if [[ $expired < $today ]]; then
		...
		mv -f /usr/local/etc/xray/ntls.json.tmp /usr/local/etc/xray/ntls.json
		cat /phantom/user_database.json | jq 'del(.xray.vless[] | select(.username == "'$user'"))' > /phantom/user_database.json.tmp
		mv /phantom/user_database.json.tmp /phantom/user_database.json
	fi
...
while read user; do
	...
	if [[ $expired < $today ]]; then
		...
		mv -f /usr/local/etc/xray/ntls.json.tmp /usr/local/etc/xray/ntls.json
		cat /phantom/user_database.json | jq 'del(.xray.vmess[] | select(.username == "'$user'"))' > /phantom/user_database.json.tmp
		mv /phantom/user_database.json.tmp /phantom/user_database.json
	fi
done <<< "$(cat /phantom/user_database.json | jq -r '.xray.vmess[].username')"
...
while read user; do
	...
	if [[ $expired < $today ]]; then
		...
		mv -f /usr/local/etc/xray/ntls.json.tmp /usr/local/etc/xray/ntls.json
		cat /phantom/user_database.json | jq 'del(.xray.trojan[] | select(.username == "'$user'"))' > /phantom/user_database.json.tmp
		mv /phantom/user_database.json.tmp /phantom/user_database.json
	fi
done <<< "$(cat /phantom/user_database.json | jq -r '.xray.trojan[].username')"
