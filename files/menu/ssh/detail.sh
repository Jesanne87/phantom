...
clear
if [[ ! $(cat /phantom/user_database.json | jq -r '.ssh[].username') ]]; then
	echo ""
...
password=$(cat /phantom/user_database.json | jq -r '(.ssh[] | select(.username == "'$username'")).password')
expired=$(cat /phantom/user_database.json | jq -r '(.ssh[] | select(.username == "'$username'")).expired')
