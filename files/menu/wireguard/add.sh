...
clear
echo ""
until [[ ! -z $username ]] && [[ ! $(cat /phantom/user_database.json | jq -r '(.wireguard[] | select(.username == "'$username'")).username') ]] && [[ $username == ?(+|-)+([a-zA-Z0-9]) ]]; do
	read -p "Username : " username
	[[ -z $username ]] && echo -e "[ERROR] Null input."
	[[ $(cat /phantom/user_database.json | jq -r '(.wireguard[] | select(.username == "'$username'")).username') ]] && echo -e "[ERROR] User already exist."
...
Endpoint = ${endpoint}
AllowedIPs = 0.0.0.0/0" >> /phantom/wireguard/${username}.conf
echo -e "\n### Client ${username}
[Peer]
...
cat /phantom/user_database.json | jq '.wireguard += [{"username": "'${username}'","expired": "'${expired}'"}]' > /phantom/user_database.json.tmp
...
echo ""
cat /phantom/wireguard/${username}.conf
echo ""
echo "Config Link: https://${installSubDomain}/wg/${username}.conf"
echo ""
echo -e "\e[4mQR Code\e[24m"
echo ""
qrencode -t ansiutf8 -l L < /phantom/wireguard/${username}.conf
echo ""
