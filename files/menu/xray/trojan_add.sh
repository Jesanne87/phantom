...
clear
echo ""
until [[ ! -z $username ]] && [[ ! $(cat /phantom/user_database.json | jq -r '(.xray.trojan[] | select(.username == "'$username'")).username') ]] && [[ $username == ?(+|-)+([a-zA-Z0-9]) ]]; do
	read -p "Username : " username
	[[ -z $username ]] && echo -e "[ERROR] Null input."
	[[ $(cat /phantom/user_database.json | jq -r '(.xray.trojan[] | select(.username == "'$username'")).username') ]] && echo -e "[ERROR] User already exist."
...
cat /phantom/user_database.json | jq '.xray.trojan += [{"username": "'${username}'","password": "'${password}'","expired": "'${expired}'"}]' > /phantom/user_database.json.tmp
