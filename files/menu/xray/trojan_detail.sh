...
password=$(cat /phantom/user_database.json | jq -r '.xray.trojan[] | select(.username == "'$username'").password')
expired=$(cat /phantom/user_database.json | jq -r '.xray.trojan[] | select(.username == "'$username'").expired')
