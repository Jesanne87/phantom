...
exp_old=$(cat /phantom/user_database.json | jq -r '(.xray.trojan[] | select(.username == "'$username'")).expired')
diff=$((($(date -d "${exp_old}" +%s)-$(date +%s))/(86400)))
duration=$(expr $diff + $days + 1)
exp_new=$(date -d +${duration}days +%Y-%m-%d)

cat /phantom/user_database.json | jq '(.xray.trojan[] | select(.username == "'$username'")).expired |= "'$exp_new'"' > /phantom/user_database.json.tmp
