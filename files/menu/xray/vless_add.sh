...
clear
echo ""
until [[ ! -z $username ]] && [[ ! $(cat /phantom/user_database.json | jq -r '(.xray.vless[] | select(.username == "'$username'")).username') ]] && [[ $username == ?(+|-)+([a-zA-Z0-9]) ]]; do
	read -p "Username : " username
	[[ -z $username ]] && echo -e "[ERROR] Null input."
	[[ $(cat /phantom/user_database.json | jq -r '(.xray.vless[] | select(.username == "'$username'")).username') ]] && echo -e "[ERROR] User already exist."
...
uuid=$(xray uuid)
while cat /phantom/user_database.json | jq -r '.xray.vless[].uuid' | grep -qw "$uuid"; do
    uuid=$(xray uuid)
done
...
cat /phantom/user_database.json | jq '.xray.vless += [{"username": "'${username}'","uuid": "'${uuid}'","expired": "'${expired}'"}]' > /phantom/user_database.json.tmp
