...
uuid=$(cat /phantom/user_database.json | jq -r '.xray.vless[] | select(.username == "'$username'").uuid')
expired=$(cat /phantom/user_database.json | jq -r '.xray.vless[] | select(.username == "'$username'").expired')
