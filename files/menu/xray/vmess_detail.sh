...
uuid=$(cat /phantom/user_database.json | jq -r '.xray.vmess[] | select(.username == "'$username'").uuid')
expired=$(cat /phantom/user_database.json | jq -r '.xray.vmess[] | select(.username == "'$username'").expired')
