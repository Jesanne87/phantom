...
repoDir="https://raw.githubusercontent.com/Jesanne87/phantom/main/"
...
net.ipv6.conf.lo.disable_ipv6 = 1" > /etc/sysctl.d/kontol.conf
sysctl -p /etc/sysctl.d/kontol.conf
...
net.ipv4.tcp_congestion_control = bbr" >> /etc/sysctl.d/kontol.conf
sysctl -p /etc/sysctl.d/kontol.conf
...
sleep 2
echo ".: Phantom Moded Script :." > /etc/issue.net
...
echo ".: Phantom Moded Script :." > /etc/db-issue.net
mkdir /etc/dropbear
...
rm -rf dnstt
mkdir /phantom/dnstt
dnstt-server -gen-key -privkey-file /phantom/dnstt/server.key -pubkey-file /phantom/dnstt/server.pub
...
screen -AmdS dnstt dnstt-server -udp :5300 -privkey-file /phantom/dnstt/server.key ns-${installSubDomain} 127.0.0.1:85

# Install UDP Custom (https://t.me/ePro_Dev_Team)
mkdir /phantom/udp-custom
wget -O /phantom/udp-custom/udp-custom "${repoDir}files/udp-custom/udp-custom"
wget -O /phantom/udp-custom/config.json "${repoDir}files/udp-custom/config.json"
chmod +x /phantom/udp-custom/udp-custom
...
echo -e "\n# IPv4 forwarding
net.ipv4.ip_forward = 1" >> /etc/sysctl.d/kontol.conf
sysctl -p /etc/sysctl.d/kontol.conf
...
sleep 2
mkdir /phantom/openvpn
wget -O /phantom/openvpn/udp.ovpn "${repoDir}files/openvpn/client-udp.ovpn"
wget -O /phantom/openvpn/tcp.ovpn "${repoDir}files/openvpn/client-tcp.ovpn"
sed -i "s/install_ip/$installIP/g" /phantom/openvpn/udp.ovpn
sed -i "s/install_ip/$installIP/g" /phantom/openvpn/tcp.ovpn
echo -e "\n<ca>" >> /phantom/openvpn/udp.ovpn
cat "/etc/openvpn/key/ca.crt" >> /phantom/openvpn/udp.ovpn
echo "</ca>" >> /phantom/openvpn/udp.ovpn
echo -e "\n<ca>" >> /phantom/openvpn/tcp.ovpn
cat "/etc/openvpn/key/ca.crt" >> /phantom/openvpn/tcp.ovpn
echo "</ca>" >> /phantom/openvpn/tcp.ovpn
...
cd
rm -rf Python-2.7.18
mkdir /phantom/websocket
wget -O /phantom/websocket/ws-dropbear.py "${repoDir}files/websocket/ws-dropbear.py"
wget -O /phantom/websocket/ws-openvpn.py "${repoDir}files/websocket/ws-openvpn.py"
wget -O /phantom/websocket/ohpserver "${repoDir}files/websocket/ohpserver"
chmod +x /phantom/websocket/ohpserver
...
if [ "$installDNS" = "cf" ]; then
	...
	dns_cloudflare_api_token =  ${installCFToken}" > /phantom/xray/cloudflare.ini
	chmod 600 /phantom/xray/cloudflare.ini
	certbot certonly --dns-cloudflare --dns-cloudflare-credentials /phantom/xray/cloudflare.ini --dns-cloudflare-propagation-seconds 30 --noninteractive --agree-tos -d $installSubDomain -d *.$installSubDomain --register-unsafely-without-email
else
    snap install certbot-dns-digitalocean
	echo "# DigitalOcean API token used by Certbot
	dns_digitalocean_token = ${installDOToken}" > /phantom/xray/digitalocean.ini
	chmod 600 /phantom/xray/digitalocean.ini
	certbot certonly --dns-digitalocean --dns-digitalocean-credentials /phantom/xray/digitalocean.ini --dns-digitalocean-propagation-seconds 30 --noninteractive --agree-tos -d $installSubDomain -d *.$installSubDomain --register-unsafely-without-email
fi
...
htpasswd -bc /etc/nginx/conf.d/.htpasswd kontol P4s5W0rD
...
iptables-save > /phantom/iptables.rules
systemctl start wg-quick@wg0
...
installOrg=\"BTVPN\"
installOrgUnit=\"BTVPN\"
installCountry=\"Malaysia\"
installProvince=\"Bundu\"
installCity=\"Kota Kinabalu\"
installDNS=\"${installDNS}\"
installDomain=\"${installDomain}\"
installSubDomain=\"${installSubDomain}\"
installIP=\"${installIP}\"
netInt=\"${netInt}\"" > /phantom/params
...
sleep 2
mkdir /phantom/log
mkdir -p /phantom/menu/{ssh,wireguard,xray,other}
wget -O /phantom/menu/ssh/add.sh "${repoDir}files/menu/ssh/add.sh"
wget -O /phantom/menu/ssh/delete.sh "${repoDir}files/menu/ssh/delete.sh"
wget -O /phantom/menu/ssh/extend.sh "${repoDir}files/menu/ssh/extend.sh"
wget -O /phantom/menu/ssh/list.sh "${repoDir}files/menu/ssh/list.sh"
wget -O /phantom/menu/ssh/detail.sh "${repoDir}files/menu/ssh/detail.sh"
wget -O /phantom/menu/ssh/dropbear_monitor.sh "${repoDir}files/menu/ssh/dropbear_monitor.sh"
wget -O /phantom/menu/ssh/openvpn_monitor.sh "${repoDir}files/menu/ssh/openvpn_monitor.sh"
wget -O /phantom/menu/wireguard/add.sh "${repoDir}files/menu/wireguard/add.sh"
wget -O /phantom/menu/wireguard/delete.sh "${repoDir}files/menu/wireguard/delete.sh"
wget -O /phantom/menu/wireguard/extend.sh "${repoDir}files/menu/wireguard/extend.sh"
wget -O /phantom/menu/wireguard/list.sh "${repoDir}files/menu/wireguard/list.sh"
wget -O /phantom/menu/wireguard/detail.sh "${repoDir}files/menu/wireguard/detail.sh"
wget -O /phantom/menu/xray/trojan_add.sh "${repoDir}files/menu/xray/trojan_add.sh"
wget -O /phantom/menu/xray/trojan_delete.sh "${repoDir}files/menu/xray/trojan_delete.sh"
wget -O /phantom/menu/xray/trojan_extend.sh "${repoDir}files/menu/xray/trojan_extend.sh"
wget -O /phantom/menu/xray/trojan_detail.sh "${repoDir}files/menu/xray/trojan_detail.sh"
wget -O /phantom/menu/xray/trojan_list.sh "${repoDir}files/menu/xray/trojan_list.sh"
wget -O /phantom/menu/xray/trojan_monitor.sh "${repoDir}files/menu/xray/trojan_monitor.sh"
wget -O /phantom/menu/xray/vless_add.sh "${repoDir}files/menu/xray/vless_add.sh"
wget -O /phantom/menu/xray/vless_delete.sh "${repoDir}files/menu/xray/vless_delete.sh"
wget -O /phantom/menu/xray/vless_extend.sh "${repoDir}files/menu/xray/vless_extend.sh"
wget -O /phantom/menu/xray/vless_detail.sh "${repoDir}files/menu/xray/vless_detail.sh"
wget -O /phantom/menu/xray/vless_list.sh "${repoDir}files/menu/xray/vless_list.sh"
wget -O /phantom/menu/xray/vless_monitor.sh "${repoDir}files/menu/xray/vless_monitor.sh"
wget -O /phantom/menu/xray/vmess_add.sh "${repoDir}files/menu/xray/vmess_add.sh"
wget -O /phantom/menu/xray/vmess_delete.sh "${repoDir}files/menu/xray/vmess_delete.sh"
wget -O /phantom/menu/xray/vmess_extend.sh "${repoDir}files/menu/xray/vmess_extend.sh"
wget -O /phantom/menu/xray/vmess_detail.sh "${repoDir}files/menu/xray/vmess_detail.sh"
wget -O /phantom/menu/xray/vmess_list.sh "${repoDir}files/menu/xray/vmess_list.sh"
wget -O /phantom/menu/xray/vmess_monitor.sh "${repoDir}files/menu/xray/vmess_monitor.sh"
wget -O /phantom/menu/other/check.sh "${repoDir}files/menu/other/check.sh"
wget -O /phantom/menu/other/update_script.sh "${repoDir}files/menu/other/update_script.sh"
wget -O /phantom/menu/other/update_package.sh "${repoDir}files/menu/other/update_package.sh"
wget -O /phantom/menu/other/web_login.sh "${repoDir}files/menu/other/web_login.sh"
find /phantom/menu/ -type f -exec chmod +x {} \;
wget -O /usr/bin/menu "${repoDir}files/menu/menu.sh"
wget -O /usr/bin/nf "${repoDir}files/nf.sh"
wget -O /usr/bin/dns "${repoDir}files/dns.sh"
wget -O /usr/bin/backup "${repoDir}files/backup.sh"
...
echo "{
    ...
    \"wireguard\" : []
}" > /phantom/user_database.json

# Configure cron
echo "[INFO] Configure cron"
sleep 2
mkdir /phantom/cron
wget -O /phantom/cron/reboot.sh "${repoDir}files/cron/reboot.sh"
wget -O /phantom/cron/hourly.sh "${repoDir}files/cron/hourly.sh"
wget -O /phantom/cron/midnight.sh "${repoDir}files/cron/midnight.sh"
sed -i "s/install_domain/${installSubDomain}/g" /phantom/cron/reboot.sh
find /phantom/cron/ -type f -exec chmod +x {} \;
(crontab -l; echo "@reboot /phantom/cron/reboot.sh") | crontab -
(crontab -l; echo "@hourly /phantom/cron/hourly.sh") | crontab -
(crontab -l; echo "@midnight /phantom/cron/midnight.sh") | crontab -
...
clear
echo "[INFO] Selamat BerVPN Gassss"
